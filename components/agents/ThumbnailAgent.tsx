...
const ThumbnailAgent: React.FC<ThumbnailAgentProps> = ({
  ...
  // Auto-run if triggered by Auto-Pilot phase
  useEffect(() => {
    if (isAutoGenerating && titleInput && !isLoading && !thumbnailUrl) {
      handleGenerate();
    }
  }, [isAutoGenerating, titleInput]);
    ...
    >
      <div className="space-y-4">
        ...
        <button
          ...
          className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold py-2 px-4 rounded-md transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
        >
          {isLoading || isAutoGenerating ? 'Đang thiết kế...' : '✨ Tạo Thumbnail (16:9)'}
        </button>

        {(isLoading || isAutoGenerating || thumbnailUrl) && (
            <div className="relative w-full aspect-video bg-gray-900/50 rounded-lg border border-gray-700 overflow-hidden group flex items-center justify-center">
